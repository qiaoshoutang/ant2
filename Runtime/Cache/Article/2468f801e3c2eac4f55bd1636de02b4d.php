<?php if (!defined('THINK_PATH')) exit();?><h3>快讯列表</h3>
      ...
      <?php if(is_array($statusArr)): foreach($statusArr as $key=>$vo): ?><option value="<?php echo ($key); ?>"
          <?php if($pageMaps['state'] == $key): ?>selected="selected"<?php endif; ?>
          ><?php echo ($vo); ?>
  ...
  <div class="m-table-mobile"><table id="table" class="m-table "><thead><tr><th width="30">选择</th><th width="30">编号</th><th width="80">标题</th><th>内容</th><th width="100">状态</th><th width="170">时间</th><th width="130">操作</th></tr></thead><tbody><?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
        <td>
        ...
        <td>
          <?php if($vo['state']==1): ?><span class="u-badge u-badge-primary">草稿</span><?php endif; ?>

          <?php if($vo['state']==2): ?><span class="u-badge u-badge-success">通过</span><?php endif; ?>
          <?php if($vo['state']==3): ?><span class="u-badge u-badge-danger">不通过</span><?php endif; ?>
        </td>
        <td><?php echo (date("Y-m-d H:i:s",$vo["time"])); ?></td>

        <td>
        <?php if($vo['state']==1): ?><a class="u-btn u-btn-primary  u-btn-small article" href="javascript:;" onclick="message_review(this)" data="<?php echo ($vo["id"]); ?>" operation='2'>通过</a>
          <a class="u-btn u-btn-danger  u-btn-small article" href="javascript:;" onclick="message_review(this)" data="<?php echo ($vo["id"]); ?>" operation='3'>不通过</a><?php endif; ?>
        <a class="u-btn u-btn-success  u-btn-small" href="<?php echo U('edit',array('id'=>$vo['id']));?>">修改</a>
